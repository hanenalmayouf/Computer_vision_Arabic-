...
**The Car Example:**
You draw a bad box for a car (IoU < 0.5). 
...
**Where to find per-class AP:**
- Check `metrics.box.ap_class_index` for class names
- `metrics.box.ap` contains AP for each class individually
...
The confusion matrix shows **what your model predicted vs. what was actually there**.
...
- Strong diagonal (high values on the diagonal)
...
- High values in wrong rows (e.g., truck predicted as car)
- Indicates: model can't distinguish between these classes

```
           pred  
           car  trck
actual car  60   40   ← Model confuse
       trck  45   35
```
:::
...
**If your 'truck' column has high values in the 'car' row, your model can't tell them apart — consider more diverse training data!**
...
|--------|-------------|-----|
| **mAP50** | Iterating fast, quick sanity check | Only requires IoU > 0.50, which is generous |
| **mAP50-95** | Production readiness | Strict — averages across 10 IoU thresholds (0.50, 0.55, ..., 0.95) |

::: {.callout-tip appearance="simple" icon="false"}
**Rule of Thumb:**
- Use **mAP50** during rapid experimentation
- Use **mAP50-95** when you're ready to ship to production
:::
...
- Train loss drops, val loss stops improving or gets worse
- Model memorized training data instead of learning general patterns
- **Key sign:** If train mAP >> val mAP, you're overfitting
:::
...
- Needs a bigger model or more data
:::

::::

**Visual (loss curves):**
- 🟢 **Healthy:** Train and val loss converge together
- 🔴 **Overfitting:** Gap between train and val loss widens
- 🟠 **Underfitting:** Both losses are flat and high


## Data Leakage: The Silent Killer

**What it is:** Validation/test data accidentally ends up in the training set.

**How it happens:**
- Same image appears in both train and val folders
- Temporal data split wrong (e.g., future data in training)
- Preprocessing fit on full dataset before splitting

**Why it's catastrophic:**
- Val metrics look great, but model fails in production
- You can't trust ANY evaluation number

**How to avoid:**
- Use `autosplit()` from Ultralytics to split your data
- Never manually copy images between train/val folders
- Check for duplicate images before training

::: {.callout-warning appearance="simple" icon="false"}
**If val metrics are suspiciously perfect, check for data leakage!**
:::
...
ax.set_facecolor('white')
draw_base_car(ax)
ax.set_xlim(16, 0)  # Flip horizontally
ax.set_title('Flip', fontsize=10, fontweight='bold')
ax.set_xticks([]); ax.set_yticks([])
for sp in ax.spines.values(): sp.set_visible(False)
...
ax.text(0.5, 0.5, 'Car', transform=ax.transAxes, fontsize=20, ha='center', va='center', color='#888', fontweight='bold')
ax.set_title('Brightness', fontsize=10, fontweight='bold')
ax.set_xticks([]); ax.set_yticks([])
for sp in ax.spines.values(): sp.set_visible(False)
...
ax.set_facecolor('white')
draw_base_car(ax)
ax.set_title('Blur', fontsize=10, fontweight='bold')
ax.set_xticks([]); ax.set_yticks([])
for sp in ax.spines.values(): sp.set_visible(False)
...
ax.set_facecolor('white')
draw_base_car(ax)
ax.set_title('Rotation', fontsize=10, fontweight='bold')
ax.set_xticks([]); ax.set_yticks([])
for sp in ax.spines.values(): sp.set_visible(False)
...
**You're teaching the model: a car is still a car upside down, in shadow, or slightly blurry.**
...
## Spotting Problems in Your Training

| Symptom | Root Cause | Fix |
|---------|------------|-----|
| Train mAP high, val mAP low | **Overfitting** | Use augmentation + increase data |
| Train mAP and val mAP both low | **Underfitting** | Use bigger model or more data |
| Val mAP suspiciously perfect | **Data Leakage** | Re-check your train/val split |

**Where to check:**
- `results.csv` (in your training run folder)
- TensorBoard curves

::: {.callout-note appearance="simple" icon="false"}
**Example:** If your train mAP is 0.85 but val mAP is 0.45, you have an overfitting problem. The model memorized the training data!
:::


## Now You Know the Problems. Here's How to Fix Them

Now that you understand overfitting and data augmentation, let's look at the hyperparameters that help:

**Coming from Part 3:**
- `epochs`, `batch`, `imgsz`, `device`, `optimizer`, config files

**New tuning knobs for fixing your model:**
- `lr0`: Adjust learning rate — lower if loss explodes
- `weight_decay`: Regularization to fight overfitting
- `patience`: Early stopping to prevent overfitting
- Data Augmentation: The Albumentations code from Part 3
...
- `lr0` (Default: 0.01): Initial Learning Rate (step size).
  ...
  - *Rule:* If loss looks like random noise, lower `lr0`.

- `weight_decay` (Default: 0.0005): L2 regularization to fight overfitting.
  - Penalizes large weights to improve generalization.
  - Increase if overfitting is detected.


## Hyperparameters: Early Stopping

- `patience` (Default: 50): If validation metrics don't improve for this many epochs, training stops automatically.
...
# Train with early stopping: stop if no improvement for 10 epochs
model.train(data="data.yaml", epochs=100, patience=10)
```
