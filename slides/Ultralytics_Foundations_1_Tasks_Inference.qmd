...
## The YOLO CLI Structure {.sdaia-dark background-color="#1C355E"}
...
- **`top1`** (0): The most likely class ID.
- **`top1conf` (0.95)**: Confidence score of the top class.
- **`probs`**: Array of probabilities for *all* possible classes.
...
Since `probs` evaluates *every* class, we can predict **multiple labels** for a single image by applying a confidence threshold (e.g., keeping all classes with `prob > 0.50`) instead of just taking the `top1` prediction.
:::
...
::: {.callout-note appearance="simple" icon="false"}
**Wait, how does it know the class?**
Behind the scenes, each of those $N$ bounding boxes *also* predicted probabilities for ALL 80 classes, just like Image Classification! The final `cls` ID shown in the output is simply the class that scored the highest probability for that specific box.
:::
...
- **`visibility`**: Whether the keypoint is visible (i.e., not occluded).
...
```bash
yolo task=obb mode=predict model=yolo11n-obb.pt source="https://ultralytics.com/images/bus.jpg"
```
